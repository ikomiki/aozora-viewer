...
青空文庫形式のテキストファイルを、ブラウザ上で美しく読みやすい形式で表示するWebアプリケーション。ドラッグ＆ドロップによる直感的なファイル読み込みと、カスタマイズ可能な読書環境を提供する。UTF-8およびShift-JIS形式の文字エンコードに対応し、自動判定により適切に表示する。

### テキスト整形指示の仕様

- **左インデント**: `［＃２字上げ］` - 行頭から指定個数分の全角空白でインデント
- **右寄せ**: `［＃地から２字上げ］` - 行末から指定個数分の全角空白で右寄せ（右端インデント）
- **ブロック字下げ**: `［＃ここから１字下げ］`〜`［＃ここで字下げ終わり］` - ブロック全体を左インデント
...
- REQ-001: システムは青空文庫形式のテキストファイルを解析し、ルビ記法（文字列《ふりがな》）を正しく表示しなければならない
  - 《》前に｜文字が存在しない場合は、一文字前の文字にルビを付ける
  - ｜文字がある場合は、｜からルビまでの範囲をベーステキストとして優先する
...
- REQ-005-EXT: システムはUTF-8およびShift-JIS形式でエンコードされたテキストファイルを読み込んで正しく表示しなければならない
- REQ-006: システムは［＃２字上げ］のような指示のある場合、行頭から指定個数分の全角空白文字でインデントしなければならない
- REQ-006-EXT: システムは［＃地から２字上げ］のような指示のある場合、行末から指定個数分の全角空白文字で右寄せ（右端インデント）しなければならない
...
- REQ-101-EXT: 文字エンコードが不明なファイルの場合、システムはUTF-8として試行し、失敗した場合はShift-JISとして読み込みを試行しなければならない
- REQ-102: ルビ範囲が不明確な場合（｜記号使用）、システムは｜から《》までをルビとして処理しなければならない
- REQ-102-EXT: ｜記号が行頭から《》までの範囲に存在しない場合、システムは《》直前の一文字をベーステキストとして処理しなければならない
- REQ-102-NEW: 同一行内でルビが複数存在し、｜記号が設定されていない《》については、各々一文字前の文字にルビを付けなければならない
...
- REQ-107: 左インデント指示（［＃○字上げ］）が検出された場合、システムは該当行の先頭に指定個数分の全角空白を挿入しなければならない
- REQ-107-EXT: 右寄せ指示（［＃地から○字上げ］）が検出された場合、システムは該当行を右端から指定個数分の全角空白でインデントして右寄せしなければならない
...
- EDGE-005: UTF-8とShift-JISの両方で読み込みに失敗した場合、文字化け警告とともに最良努力での表示を行う
...
- EDGE-108: 左インデント指示（［＃○字上げ］）と右寄せ指示（［＃地から○字上げ］）が同一行に存在する場合の処理
- EDGE-109: 右寄せ指示で異常に大きな数値が指定され、行内容より大きくなった場合の処理
- EDGE-110: バイナリファイルが誤ってドロップされた場合の文字エンコード検出と処理
- EDGE-111: UTF-8 BOMありファイルとBOMなしファイルの適切な処理
...
- [ ] 3つのテーマの切り替え確認
- [ ] localStorage保存・復元機能
- [ ] UTF-8形式テキストファイルの正常な読み込みと表示
- [ ] Shift-JIS形式テキストファイルの正常な読み込みと表示
- [ ] 文字エンコード自動検出機能の動作確認
...
- [ ] ネットワーク切断時の画像表示処理
- [ ] 文字化けファイルの適切なエラー表示と警告メッセージ
- [ ] バイナリファイル誤ドロップ時のエラーハンドリング
- [ ] エンコード検出失敗時のフォールバック機能